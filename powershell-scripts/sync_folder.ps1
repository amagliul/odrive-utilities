...
function get_folder($initialDirectory)
{
    $folderform = New-Object System.Windows.Forms.FolderBrowserDialog
	$result = $folderform.ShowDialog()
	if($result -eq "OK") {
        $foldername = $folderform.SelectedPath
		[void]$folderform.dispose
		return $foldername
    }
    else {
	[void]$folderform.dispose
